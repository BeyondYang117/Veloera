...
const LogsTable = () => {
  ...
  const getDefaultColumnVisibility = () => {
    return {
      ...
      [COLUMN_KEYS.PROMPT]: isAdminUser,
      [COLUMN_KEYS.COMPLETION]: isAdminUser,
  ...
  const getVisibleColumns = () => {
    return allColumns.filter((column) => {
      // 如果不是管理员，过滤掉敏感列（输入输出相关）
      if (!isAdminUser && (
          column.dataIndex === 'prompt' || 
          column.dataIndex === 'completion' ||
          column.key === COLUMN_KEYS.PROMPT ||
          column.key === COLUMN_KEYS.COMPLETION
        )) {
        return false;
      }
      return visibleColumns[column.key || column.dataIndex];
    });
  };
  ...
  const setLogsFormat = (logs) => {
    ...
    for (let i = 0; i < logs.length; i++) {
      ...
      // 如果不是管理员用户，过滤掉敏感数据
      if (!isAdminUser) {
        expandDataLocal = expandDataLocal.filter(item => 
          !['输入', '输出', '系统提示', '上下文'].includes(item.key)
        );
      }
